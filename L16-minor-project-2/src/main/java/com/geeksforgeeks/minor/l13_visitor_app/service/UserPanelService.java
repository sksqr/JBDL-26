...
public class UserPanelService {
    ...
    public void rejectVisit(Long userId, Long visitId){
        ...
        String key = "flatId_"+visit.getFlat().getId()+":"+VisitStatus.PENDING.toString();
    ...
    public List<VisitDTO> getPendingVisits(Long userId){
        ...
        String key = "flatId_"+flat.getId()+":"+VisitStatus.PENDING.toString();
        List<VisitDTO> visitDTOList = (List<VisitDTO>) redisTemplate.opsForValue().get(key);
        if(visitDTOList != null){
            return visitDTOList;
        }
        visitDTOList =  visitService.getVisitsByFlatAndStatus(flat,VisitStatus.PENDING);
